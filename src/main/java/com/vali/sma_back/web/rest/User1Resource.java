...
public class User1Resource {
    ...
    @GetMapping("/user-1-s")
    @Timed
    public List<User1DTO> getAllUser1S(@RequestParam(required = false) String filter) {
        if ("message-is-null".equals(filter)) {
            log.debug("REST request to get all User1s where message is null");
            return user1Service.findAllWhereMessageIsNull();
        }
        if ("rating-is-null".equals(filter)) {
            log.debug("REST request to get all User1s where rating is null");
            return user1Service.findAllWhereRatingIsNull();
        }
